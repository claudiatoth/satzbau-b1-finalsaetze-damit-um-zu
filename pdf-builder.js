// PDF BUILDER — Atelier A2: Arbeit & Job
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            ...
            <div class="cast-grid">
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Învață pentru examen</div></div>
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += choiceBlock('Übung 1 — damit sau um … zu?', 'zu + Infinitiv și același subiect → um. Subiect diferit → damit.', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillPromptBlock('Übung 2 — Pune verbul la locul corect', 'damit → verb conjugat la final · um … zu → zu + Infinitiv la final (la verbe separabile: einzukaufen).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillPromptBlock('Übung 3 — Transformă damit → um … zu', 'Același subiect în ambele propoziții → poți folosi um … zu.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillPromptBlock('Übung 4 — Besonderheiten (repară modalul)', 'wollen/möchten/sollen NU au voie → können sau scoase. müssen rămâne.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE (Finalsätze)', 'Construiește propoziția de scop potrivită (damit / um … zu).', ex5Data);
        }
        return html;
    }
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 4. Verbele modale — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    <div class="speaker">📌 Andreea îți spune:</div>
                    <div>Toate modalele sunt neregulate și fac Perfekt cu haben. Cheia lecției: în Finalsatz, wollen/möchten/sollen → können (sau scoase), dar müssen rămâne („um nicht zu Fuß gehen zu müssen").</div>
                </div>
            </div>`;
        if (typeof verbsData !== 'undefined') {
            verbsData.forEach((v, idx) => {
                ...
                else if (v.typ && (v.typ.indexOf('neregulat') === 0 || v.typ.indexOf('modal') === 0)) { badgeClass = 'strong'; badgeLabel = 'NEREGULAT (modal)'; }
